...
impl Component for App {
	fn render(&self) -> wasm_react::VNode {
		...
		let element_container = use_js_ref(None);

		use_effect({
			let element_container = element_container.clone();

			move || {
				element_container.current().and_then(|element: Element| element.dyn_into::<HtmlElement>().ok()).map(|element| element.focus().ok());
		...
		h!(div)
			...
			.style(
				...
				.build(c![
					...
					h!(div)
						.class_name("tile")
						.key(Some(format!("grid-{}-{}", pos.0, pos.1).to_string()))
